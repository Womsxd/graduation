...
class ViewSubject(QWidget, Ui_Form):
    now_pages = 1

    def __init__(self):
        super(ViewSubject, self).__init__()
        self.setObjectName("ViewSubject")
        self.setupUi(self)
        self.label_title.setText("科目管理")
        self.TableWidget.setColumnCount(2)
        self.TableWidget.setWordWrap(False)
        self.TableWidget.verticalHeader().hide()
        self.TableWidget.setHorizontalHeaderLabels(['编号', '科目名称'])
    ...
    def get_server_data(self):
        ...
        resp = http.get(setting.SUBJECT_LIST, params=params)
        ...
        try:
            ...
            return
        self.flush_table(data["data"]["subjects"])
    ...
    def add(self):
        w = my_dialog.MessageBoxForAdd("科目管理", "请输入科目名称", self)
        if w.exec():
            adds = w.get_input()
            if adds == "":
                MessageBox("科目管理", "输入不可为空", self).exec()
            else:
                resp = http.post(setting.SUBJECT_ADD, data={"name": adds})
                if resp.status_code != 200:
                    MessageBox("成绩管理系统", "接口请求错误", self).exec_()
                    return
                try:
    ...
    def show_context_menu(self, pos):
        ...
        if utils.check_permission("subject.edit"):
            menu.addAction("编辑")
        if utils.check_permission("subject.delete"):
            menu.addAction("删除")
        ...
        if action:
            if action.text() == "编辑":
                ...
                if selected_items:
                    ...
                    w = my_dialog.MessageBoxForAdd("科目管理", "请进行修改", self)
                    w.line_edit_name.setText(self.TableWidget.item(current_row, 1).text())
                    if w.exec():
                        adds = w.get_input()
                        if adds != "":
                            resp = http.post(setting.SUBJECT_EDIT,
            ...
            if action.text() == "删除":
                ...
                if selected_items:
                    ...
                    if w.exec():
                        resp = http.post(setting.SUBJECT_DELETE,
                                         data={"id": int(self.TableWidget.item(current_row, 0).text())})
                        if resp.status_code != 200:
                            MessageBox("成绩管理系统", "接口请求错误", self).exec_()
                            return
