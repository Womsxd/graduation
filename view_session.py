...
class ViewSession(QWidget, Ui_Form):
    now_pages = 1

    def __init__(self):
        super(ViewSession, self).__init__()
        self.setObjectName("ViewSession")
        self.setupUi(self)
        self.label_title.setText("考试信息管理")
        self.TableWidget.setColumnCount(2)
        self.TableWidget.setWordWrap(False)
        self.TableWidget.verticalHeader().hide()
        self.TableWidget.setHorizontalHeaderLabels(['编号', '考试名称'])
        ...
        if not utils.check_permission("subject.add"):
    ...
    def get_server_data(self):
        ...
        resp = http.get(setting.SESSION_LIST, params=params)
        ...
        try:
            ...
            return
        self.flush_table(data["data"]["sessions"])
    ...
    def add(self):
        w = my_dialog.MessageBoxForAdd("考试管理", "请输入考试名称", self)
        if w.exec():
            adds = w.get_input()
            if adds == "":
                MessageBox("考试管理", "输入不可为空", self).exec()
            else:
                resp = http.post(setting.SESSION_ADD, data={"name": adds})
                if resp.status_code != 200:
                    MessageBox("成绩管理系统", "接口请求错误", self).exec_()
                    return
                try:
                ...
                if data["code"] == 10001:
                    ...
                    MessageBox("考试管理", "添加成功", self).exec_()
                    self.get_server_data()

    def show_context_menu(self, pos):
        menu = DWMMenu(parent=self.TableWidget)
        if utils.check_permission("exam.session.edit"):
            menu.addAction("编辑")
        if utils.check_permission("exam.session.delete"):
            menu.addAction("删除")
        ...
        if action:
            if action.text() == "编辑":
                ...
                if selected_items:
                    ...
                    w = my_dialog.MessageBoxForAdd("考试管理", "请进行修改", self)
                    w.line_edit_name.setText(self.TableWidget.item(current_row, 1).text())
                    if w.exec():
                        adds = w.get_input()
                        if adds != "":
                            resp = http.post(setting.SESSION_EDIT,
            ...
            if action.text() == "删除":
                ...
                if selected_items:
                    current_row = selected_items[0].row()
                    w = MessageBox("考试管理", f"是否删除 {self.TableWidget.item(current_row, 1).text()}", self)
                    if w.exec():
                        resp = http.post(setting.SESSION_DELETE,
                                         data={"id": int(self.TableWidget.item(current_row, 0).text())})
                        if resp.status_code != 200:
                            MessageBox("成绩管理系统", "接口请求错误", self).exec_()
                            return
